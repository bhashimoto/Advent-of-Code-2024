...
def is_possible(target:int, acc:int, eq:list[int]) -> bool:
    ...
    concatenation = int(str(acc) + str(eq[0]))

    return is_possible(target, addition, eq[1:]) or is_possible(target, multiplication, eq[1:]) or is_possible(target, concatenation, eq[1:])
